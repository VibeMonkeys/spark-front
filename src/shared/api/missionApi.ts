...
export const missionApi = {
  ...
  // 일일 미션 시작 제한 정보 조회
  getDailyMissionLimit: async (userId: number): Promise<DailyMissionLimit> => {
    const response = await api.get<ApiResponse<DailyMissionLimit>>(`/missions/daily-limit?userId=${userId}`);
    return response.data.data;
  },

  // 미션 상세 조회
  getMissionDetail: async (missionId: number): Promise<MissionDetailResponse> => {
    try {
  ...
  // 미션 시작
  startMission: async (missionId: number, userId: number): Promise<Mission> => {
    const response = await api.post<ApiResponse<Mission>>(`/missions/${missionId}/start?userId=${userId}`);
    return response.data.data;
  },

  // 미션 진행도 업데이트
  updateMissionProgress: async (missionId: number, userId: number, progress: number): Promise<Mission> => {
  ...
  // 미션 완료
  completeMission: async (missionId: number, userId: number): Promise<MissionCompletionResponse> => {
    const response = await api.post<ApiResponse<MissionCompletionResponse>>(`/missions/${missionId}/complete?userId=${userId}`);
    return response.data.data;
  },

  // 진행 중인 미션 조회
  getOngoingMissions: async (userId: number): Promise<Mission[]> => {
  ...
  // 미션 리롤 (새로운 미션으로 교체)
  rerollMissions: async (userId: number): Promise<Mission[]> => {
    const response = await api.post<ApiResponse<Mission[]>>(`/missions/reroll?userId=${userId}`);
    return response.data.data;
  },

  // 카테고리별 통계 조회
  getCategoryStatistics: async (userId: number): Promise<CategoryStatResponse[]> => {
